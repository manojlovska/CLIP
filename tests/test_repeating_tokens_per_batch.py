...
for i, batch in enumerate(pbar):
    ...
    # Get unique rows
    unique_val = np.unique(texts, axis=0).shape[0]

    # import pdb
    # pdb.set_trace()

    n_unique_captions_per_batch = unique_val # torch.unique(texts, dim=0, return_inverse=False).shape[0]
    # repeating_captions_per_batch = Counter(captions)

    train_dict[i] = n_unique_captions_per_batch
...
with open("tests/results/train_dict.json", "w") as file:
...
for i, batch in enumerate(v_pbar):
    ...
    n_unique_captions_per_batch = len(set(val_captions))
    # repeating_captions_per_batch = Counter(val_captions)

    val_dict[i] = n_unique_captions_per_batch
...
print("Writing to a json file ... ")
with open("tests/results/val_dict.json", "w") as file:
